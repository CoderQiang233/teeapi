...
class Pay_Engine_Wechat extends Pay_Base {
    ...
    public function buildRequestForm($data) {
        ...
        if(isset($data['notify_url']) && $data['notify_url'] != ""){
            $this->param['notify_url'] = $data['notify_url'];
        }else{
            $this->param['notify_url'] = $this->config['notify_url'];
        }
        DI()->logger->debug("notify_url",array("notify_url" => $this->param['notify_url']));
        $this->param['trade_type'] = 'JSAPI';
        $this->param['openid'] = $data['openid'];

        DI()->logger->debug("this->param",array("param" =>$this->param));
