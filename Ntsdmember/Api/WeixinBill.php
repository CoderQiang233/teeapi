                    ...
                    for($i=0;$i<count($data);$i++){
                        ...
                        for($j=0;$j<count($weixinBillData['bill']);$j++){
                            ...
                            if( trim($order['order_id'])==trim($bill['order_sn_sh'])){
                                ...
                                DI()->logger->log('checkBillInventory','对账单后加入库存信息',json_encode($order));
                                $inventory->allotInventory($order);
                                DI()->logger->log('checkBillInventory','对账单后加入库存信息完成',date('Y-m-d H:i:s'));
                                //调用返现方法
                                $inventory->recordCashBackOrder($order['id']);
                            }
                        }
                    }
