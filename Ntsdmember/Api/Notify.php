...
class Api_Notify extends PhalApi_Api {
    ...
    if (empty($post) || $post == null || $post == '') {
        ...
        if(DI()->pay->verifyNotify($notify) == true){
            ...
            //获取支付记录表信息
            $rs=DI()->notorm->payment_record->where('order_id',$info['out_trade_no'])->fetchOne();

            if($rs){//如果存在，则为多次支付
                $member_order=DI()->notorm->members_order->where('id',$rs['member_order_id'])->fetchOne();

                //1.更新支付明细表
                $res=DI()->notorm->payment_record->where('order_id',$info['out_trade_no'])->update(array('state'=>'1','update_time'=>date('Y-m-d H:i:s')));

                DI()->logger->log('updatePaymentRecord','更新支付明细表',json_encode($res));

                //实际支付金额
                $payment=$member_order['payment']+$info['money'];
              
              
              
                 if($member_order['level_price']>=$payment){
                    //2.更新会员订单表
                    DI()->notorm->members_order->where('id',$member_order['id'])->update(array('payment'=>$payment));

                    DI()->logger->log('多次支付','时间'.date('Y-m-d H:i:s'),'已付金额|应付金额'.json_encode($payment).'|'.json_encode($member_order['level_price']));

                    DI()->logger->info('多次支付应付金额和已付金额比较'.json_encode($member_order['level_price']==$payment));

                }

                if($member_order['level_price']==$payment){

                    DI()->logger->log('多次支付','时间'.date('Y-m-d H:i:s'),'调用更新方法'.json_encode($member_order['order_id']));

                    $res = $this->updateOrder($member_order['order_id']);
                }else{
                    $res=true;
                }
                ...
                $member_order=DI()->notorm->members_order->where('order_id',$info['out_trade_no'])->fetchOne();

                DI()->notorm->members_order->where('id',$member_order['id'])->update(array('payment'=>$payment));

                DI()->logger->log('一次性支付','时间'.date('Y-m-d H:i:s'),'支付金额'.json_encode($payment));

                DI()->logger->log('一次性支付','时间'.date('Y-m-d H:i:s'),'调用更新方法');
    ...
    function checkcode($data){
        ...
        $result=$members->where(array("authorization_number"=>$data,"flag"=>1))->fetchOne();
        DI() -> logger -> error('查询结果: ',$result);
        return $result;

    }

    function authorization_number(){
        ...
        if($this->checkcode($this->getcode($authorization_number))){
            return $this->authorization_number();

        }else{
            return $this->getcode($authorization_number);
        }
    ...
    private function updateOrder($out_trade_no){
		DI()->logger->log('調更新111','时间'.date('Y-m-d H:i:s'),json_encode($out_trade_no));
        try{
